...
public class IngestionEngine extends InternalEngine {
     ...
     * 2. If found, refreshing the internal reader to ensure the write is visible
     * 3. Reading the document from Lucene
     *
    ...
    public BytesReference getSourceForPartialUpdate(String id) {
        ...
        try (Releasable ignore = versionMap.acquireLock(uidTerm.bytes())) {
            ...
            if (versionValue != null) {
                ...
                // Refresh the internal reader to ensure we see this write.
                // Note: We use localCheckpointTracker.getProcessedCheckpoint() instead of versionValue.seqNo
                // because IngestionEngine doesn't assign proper seqNos to operations (they use UNASSIGNED_SEQ_NO).
                // Using the processed checkpoint ensures we refresh up to all completed writes.
