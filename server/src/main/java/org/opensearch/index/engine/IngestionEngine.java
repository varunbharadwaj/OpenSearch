...
public class IngestionEngine extends InternalEngine {
    ...
    private IndexResult indexIntoLucene(Index index) throws IOException {
        boolean isAppendOnlyIndex = engineConfig.getIndexSettings().getIndexMetadata().isAppendOnlyIndex();
        if (index.getAutoGeneratedIdTimestamp() != IndexRequest.UNSET_AUTO_GENERATED_TIMESTAMP || isAppendOnlyIndex) {
