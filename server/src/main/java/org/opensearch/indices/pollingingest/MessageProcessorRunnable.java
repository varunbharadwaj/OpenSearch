...
public class MessageProcessorRunnable implements Runnable, Closeable {
    ...
    static class MessageProcessor {
        ...
        protected MessageOperation getOperation(ShardUpdateMessage shardUpdateMessage, MessageProcessorMetrics messageProcessorMetrics)
            ...
            switch (opType) {
                ...
                case CREATE:
                    ...
                    break;
                case UPDATE:
                    operation = createPartialUpdateOperation(
                        payloadMap,
                        id,
                        pointer,
                        documentVersion,
                ...
                case DELETE:
                    if (shardUpdateMessage.autoGeneratedIdTimestamp() != UNSET_AUTO_GENERATED_TIMESTAMP) {
                        logger.warn("Delete operation without ID received, and will be dropped.");
                        messageProcessorMetrics.invalidMessageCounter.inc();
        ...
        /**
         * Creates an Index operation for INDEX and CREATE op types.
         */
        private Engine.Index createIndexOperation(
        ...
        /**
         * Creates a partial update operation by:
         * 1. Reading the existing document from the index
         * 2. Merging the partial update with the existing document
         * 3. Creating an Index operation with the merged source
         *
         * If the document doesn't exist, the partial update is treated as a full document creation.
         * This is similar to the doc_as_upsert behavior in the push-based update flow.
         *
         * @param payloadMap the parsed payload containing the partial update
         * @param id the document ID
         * @param pointer the shard pointer for tracking offset
         * @param documentVersion the external version (if specified)
         * @param documentVersionType the version type
         * @param messageProcessorMetrics metrics for tracking
         * @return the Index operation with merged document
         */
        @SuppressWarnings("unchecked")
        private Engine.Index createPartialUpdateOperation(
            Map<String, Object> payloadMap,
            String id,
            IngestionShardPointer pointer,
            long documentVersion,
            ...
            if (!payloadMap.containsKey(SOURCE)) {
                messageProcessorMetrics.invalidMessageCounter.inc();
                String errorMessage = "Missing _source field for partial update. Invalid message";
                logger.error(errorMessage);
                throw new IllegalArgumentException(errorMessage);
            }
            if (!(payloadMap.get(SOURCE) instanceof Map)) {
                messageProcessorMetrics.invalidMessageCounter.inc();
                String errorMessage = "_source field does not contain a map for partial update. Invalid message";
                logger.error(errorMessage);
                throw new IllegalArgumentException(errorMessage);
            }

            Map<String, Object> partialSource = (Map<String, Object>) payloadMap.get(SOURCE);

            // Get the existing document source
            BytesReference existingSourceBytes = engine.getSourceForPartialUpdate(id);
            ...
            if (existingSourceBytes != null) {
                ...
                // This recursively merges nested maps
                XContentHelper.update(mergedSource, partialSource, false);

                logger.debug("Merged partial update for document [{}]", id);
            } else {
                // Document doesn't exist - treat partial update as full document (upsert behavior)
                logger.debug("Document [{}] not found for partial update, treating as upsert", id);
                mergedSource = partialSource;
            }

            // Create the index operation with merged source
            BytesReference mergedSourceBytes = convertToBytes(mergedSource);
            SourceToParse sourceToParse = new SourceToParse(
                index,
                id,
                mergedSourceBytes,
                MediaTypeRegistry.xContentType(mergedSourceBytes),
                null
            );
            ParsedDocument doc = engine.getDocumentMapperForType().getDocumentMapper().parse(sourceToParse);
            ParseContext.Document document = doc.rootDoc();
            ...
            return new Engine.Index(
                ...
                Engine.Operation.Origin.PRIMARY,
                System.nanoTime(),
                UNSET_AUTO_GENERATED_TIMESTAMP, // partial updates always have explicit IDs
                false,
                UNASSIGNED_SEQ_NO,
                0
            );
        ...
        while (Thread.currentThread().isInterrupted() == false && closed == false) {
            ...
            if (shardUpdateMessage != null) {
                try {
                    ...
                    // Messages with version conflicts will be dropped. For partial updates, this means the incoming
                    // update has an older version than the current document and should be ignored. For full documents,
                    // this ensures idempotency when the same message is processed multiple times.
