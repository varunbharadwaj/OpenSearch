...
public class TransportResumeIngestionAction extends TransportClusterManagerNodeAction<ResumeIngestionRequest, ResumeIngestionResponse> {
    ...
    protected void clusterManagerOperation(
        ...
        ingestionStateService.updateIngestionPollerState("resume-ingestion", concreteIndices, updateIngestionStateRequest, new ActionListener<>(){

            @Override
            public void onResponse(UpdateIngestionStateResponse updateIngestionStateResponse) {
                boolean shardsAcked = updateIngestionStateResponse.isAcknowledged() && updateIngestionStateResponse.getTotalShards() > 0 && updateIngestionStateResponse.getFailedShards() == 0;
                ResumeIngestionResponse response = new ResumeIngestionResponse(true, shardsAcked, updateIngestionStateResponse.getShardFailureList(), updateIngestionStateResponse.getErrorMessage());
